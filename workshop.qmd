...
Last week's election saw surprising swings away from the Democrats at the Presidential level. Blue stronghold states, including New York, swung to the right. It is useful to place these political movements within the broader historical context of US Presidential elections.

Today, we will use R to collect, explore, and communicate data on US Presidential Elections.
...
We will start by collecting data on the 2020 US Presidential state-level returns. The [MIT Election Data + Science Lab](https://electionlab.mit.edu/data) provides this. To access their data, we will use the Harvard Dataverse API. We will access it using the R package `dataverse`.
...
This code programmatically pulls the most up-to-date data set from the API. Once the MIT Election Lab publishes the 2024 Presidential Election data, you will be able to access it without having to update your code.
...
I am going to save a copy of the latest US Presidential Election results. I am going to include in the file name the date that I pulled the data. This prevents me from overwriting previous versions of the data. I need those previous versions if I want to track changes made.
...
To make sure you are reading in the latest version, you can run the following:
...
Let's make sure that we have all years covered. US Presidential elections occur every four years, so we should have `r ((max_year - min_year) / 4) + 1` unique years in our dataset. Let's confirm that:
...
This is a bit unweildy. Let's visualize it to get a better sense:
...
Partially, yes. There is a huge spike in the number of candidates written in to the ballot. There also appears to be an increase in the number of candidates on the ballot; however, the appears to be following a general trend across time.
...
## Focusing on the presidential races
...
Let's focus on individual states:
...
::: {.callout-note}
I will shortly be referring to a new language called **markdown**. If you are not familiar with markdown, I highly recommend this short tutorial: [https://www.markdowntutorial.com/](https://www.markdowntutorial.com/). 
:::
